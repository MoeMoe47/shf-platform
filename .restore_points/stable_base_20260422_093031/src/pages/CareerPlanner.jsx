...
import { useCreditCtx } from "@/context/CreditContext.jsx"; // ← NEW
...
function loadImpactOverride() {
  try { return JSON.parse(localStorage.getItem(IMPACT_LS_KEY) || "null"); } catch { return null; }
}
    ...
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label={title || "Dialog"}
           ...
           style={{
             ...
             width:"min(880px, 92vw)", background:"#fff", border:"1px solid var(--ring)", borderRadius:12,
             boxShadow:"0 20px 60px rgba(0,0,0,.25)", zIndex:90, display:"flex", flexDirection:"column"
           }}>
        <header style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:12, borderBottom:"1px solid var(--ring)" }}>
  ...
  return (
    <section className="card card--pad" aria-label="Program impact" style={{ border:"1px solid var(--ring)", background:"var(--card,#fff)" }}>
      <div className="sh-row" style={{ alignItems:"center", marginBottom:8 }}>
        ...
        <div style={{ flex:1 }} />
        {overridden && <span className="sh-chip" style={{ marginRight:8 }}>Local</span>}
      ...
      <div className="impactGrid" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:10 }}>
        {safe.map((k, i) => (
          <div key={i} role="group" aria-label={`${k.label} ${k.value}`}
               style={{ border:"1px solid var(--ring)", borderRadius:12, padding:12, background:"#fff" }}>
            <div className="sh-muted" style={{ fontSize:12 }}>{k.label}</div>
            <div style={{ fontWeight:800, fontSize:20, lineHeight:1.2 }}>{k.value}</div>
          </div>
        ))}
      </div>
      <p className="subtle" style={{ marginTop:8 }}>{footnote || "Figures are cohort medians; results may vary."}</p>
    </section>
  );
}
...
export default function CareerPlanner() {
  ...
  // Load + persist funding plan
  useEffect(() => { try { const raw = localStorage.getItem(FUNDING_LS_KEY); if (raw) setFundingPlan(JSON.parse(raw)); } catch {} }, []);
  useEffect(() => { try { fundingPlan ? localStorage.setItem(FUNDING_LS_KEY, JSON.stringify(fundingPlan)) : localStorage.removeItem(FUNDING_LS_KEY); } catch {} }, [fundingPlan]);
  ...
  const handlePersonalizerSave = (ans) => { setInputs(ans || null); try { track("plan_personalizer_saved", { via:"onSave" }); } catch {} };
    ...
    <div className="sh-grid railGrid" style={{ gridTemplateColumns:"1fr" }}>
      ...
      <div className="card card--pad">
        <div className="sh-row" style={{ alignItems:"center" }}>
          ...
          <button className="sh-btn sh-btn--soft" onClick={toggleAdmin} style={{ marginLeft:8 }}
                  title="Toggle Admin (persists in this browser)" aria-pressed={admin ? "true" : "false"}>
        ...
        <div className="card card--pad">
          ...
          <Suspense fallback={<CardSkel />}>
            <FundingWizard
              onSave={(fp) => {
                setFundingPlan(fp);
                try { track("funding_plan_saved", { programsCount: fp?.steps?.length || 0 }); } catch {}

                // --- SHF credit + event (NEW) ---
                try {
                  credit?.earn?.({
                    action: "funding.plan.saved",
                    rewards: { corn: 2 },
                    scoreDelta: 4,
                    meta: { programsCount: fp?.steps?.length || 0 }
                  });
                  window.dispatchEvent(new CustomEvent("funding:plan:saved", {
                    detail: { programsCount: fp?.steps?.length || 0 }
                  }));
                  window.shToast?.("💸 Funding plan saved · +2 🌽 · +4 score");
                } catch {}
              }}
            />
          </Suspense>
...
/* --- SHF: Career plan saved (event hook) --- */
(() => {
  ...
  window.addEventListener("career:plan:saved", (e) => {
    ...
    try {
      window.shfCredit?.earn?.({
        action: "career.plan.save",
        rewards: { wheat: 2 },
        scoreDelta: 5,
        meta: { planId: d.planId }
      });
