...
import SystemSummaryCards from "@/components/exchange/operator/SystemSummaryCards";
import {
  getOperatorSummary,
  getOperatorFlow,
  getOperatorDisputes,
...
export default function OperatorControlPanel() {
  ...
  const [aalRisk,setAalRisk] = useState([])
  const [aalPrograms,setAalPrograms] = useState([])
  const [aalAnomalies,setAalAnomalies] = useState([])

  async function loadAAL() {
    try {

      const risk = await fetchJson(`${API_BASE}/api/v1/aal/participant-risk`)
      const programs = await fetchJson(`${API_BASE}/api/v1/aal/program-health`)
      const anomalies = await fetchJson(`${API_BASE}/api/v1/aal/anomalies`)

      setAalRisk(risk.items || [])
      setAalPrograms(programs.items || [])
      setAalAnomalies(anomalies.items || [])

    } catch (err) {
      console.warn("AAL load failed",err)
    }
  }
...
async function loadDashboard() {
  ...
  async function loadPoolDetail(poolId) {
    ...
    setError("");
    try {
      const res = await getOperatorPool(poolId);
      setPoolDetail(res);
    } catch (err) {
      setPoolDetail(null);
  ...
  async function loadPayoutDetail() {
    ...
    setError("");
    try {
      const res = await getOperatorPayout(payoutIntentId);
      setPayoutDetail(res);
    } catch (err) {
  ...
  async function loadTreasuryAccount() {
    ...
    setError("");
    try {
      const res = await getTreasuryAccount(treasuryCode);
      setTreasuryData(res);
    } catch (err) {
    ...
    <div style={{ display: "grid", gap: 20 }}>
      ...
      {loading ? (
        ...
        <>
          ...
          >
            <SystemSummaryCards summary={summary} />
          </Card>
          ...
          <Card title="Participant Risk Monitor">
              ...
              <tbody>
                {aalRisk.map(r=>(
                  <tr key={r.risk_id}>
                    ...
                    <td>{r.risk_band}</td>
                    <td>{r.dropout_risk_score}</td>
                    <td>{r.completion_probability}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <Card title="Program Health Monitor">
            <table style={{width:"100%"}}>
              <thead>
                <tr>
                  <th>Program</th>
              ...
              <tbody>
                {aalPrograms.map(p=>(
                  <tr key={p.snapshot_id}>
                    <td>{p.program_id}</td>
                    <td>{p.program_health_score}</td>
                    <td>{p.verified_outcome_rate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <Card title="AAL Anomaly Monitor">
            <table style={{width:"100%"}}>
              <thead>
                <tr>
                  <th>Type</th>
