...
export default function PartnerActionQueue() {
  ...
  const summary = useMemo(() => {
    ...
    const assigned = items.filter((item) => String(item.status || "").toLowerCase() === "assigned").length;
    const inReview = items.filter((item) => String(item.status || "").toLowerCase() === "in_review").length;
    const resolved = items.filter((item) => String(item.status || "").toLowerCase() === "resolved").length;
    const closed = items.filter((item) => String(item.status || "").toLowerCase() === "closed").length;
    const highPriority = items.filter((item) => String(item.urgency_level || item.priority || "").toLowerCase() === "high").length;
    const unassigned = items.filter((item) => !item.assigned_user_id).length;

    return { total, open, assigned, inReview, resolved, closed, highPriority, unassigned };
  }, [items]);
          ...
          <div className="ar-grid" style={{ marginTop: 0, marginBottom: 18 }}>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">Open</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.open}</span></div>
              </div>
            </div>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">Assigned</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.assigned}</span></div>
              </div>
            </div>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">In Review</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.inReview}</span></div>
              </div>
            </div>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">High Priority</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.highPriority}</span></div>
              </div>
            </div>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">Unassigned</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.unassigned}</span></div>
              </div>
            </div>
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow"><div className="ar-name">Completed</div></div>
                <div className="ar-meta"><span className="ar-mono">{summary.resolved + summary.closed}</span></div>
              </div>
            </div>
          </div>
