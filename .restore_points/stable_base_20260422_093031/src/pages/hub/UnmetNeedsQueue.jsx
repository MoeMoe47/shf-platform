...
export default function UnmetNeedsQueue() {
  ...
  const metrics = useMemo(() => {
    ...
    const highPriority = referrals.filter(
      (r) => String(r.priority || r.urgency_level || "").toLowerCase() === "high"
    );
    const unassigned = referrals.filter((r) => !r.assigned_user_id);
    const likelyUnmet = referrals.filter((r) => {
      const status = String(r.status || "").toLowerCase();
      const priority = String(r.priority || r.urgency_level || "").toLowerCase();
      return status === "open" && priority === "high";
    });

    const onHold = referrals.filter((r) => String(r.status || "").toLowerCase() === "on_hold");
        ...
        <>
          <div className="ar-grid" style={{ marginTop: 0, marginBottom: 18 }}>
            ...
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow">
                  <div className="ar-name">High Priority Referrals</div>
                  <span className={severityPillClass("high")}>
                    {metrics.highPriorityCount}
                  </span>
                </div>
                <div className="ar-meta">
                  <span className="ar-mono">Urgent demand across referral stream</span>
                </div>
              </div>
              <div className="ar-body">
                <div className="ar-row">
                  <div className="ar-label">Meaning</div>
                  <div className="ar-value">
                    High-priority referrals are the strongest early indicator of unmet-need pressure.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="ar-card">
              <div className="ar-top">
                <div className="ar-nameRow">
                  ...
                  <span className="rg-pill rg-pillOk">{metrics.onHoldCount}</span>
                </div>
                <div className="ar-meta">
                  <span className="ar-mono">Paused referrals with unresolved blockers</span>
                </div>
              </div>
              <div className="ar-body">
                <div className="ar-row">
                  <div className="ar-label">Meaning</div>
                  <div className="ar-value">
                    On-hold cases often signal capacity, documentation, or coordination gaps.
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="ar-body">
                {metrics.likelyUnmet.length === 0 ? (
                  ...
                  metrics.likelyUnmet.map((item) => {
                    const urgency = item.priority || item.urgency_level || "low";
                    return (
                      <div
                        key={item.case_id}
                        style={{
            ...
            <div className="ar-card">
              ...
              <div className="ar-body">
                <div className="ar-row">
                  ...
                  <div className="ar-value">
                    This page currently uses referral status and priority as an early unmet-need proxy.
                  </div>
                </div>
                <div className="ar-row">
                  <div className="ar-label">Next Upgrade</div>
                  <div className="ar-value">
                    Add true unmet-need objects, need-category rollups, barrier reasons, and explicit capacity constraints from the backend.
                  </div>
                </div>
                <div className="ar-row">
                  <div className="ar-label">Why It Matters</div>
                  <div className="ar-value">
                    Even early inference helps leadership see demand pressure, unresolved high-priority needs, and service-capacity gaps.
                  </div>
                </div>
                <div className="ar-row">
                  <div className="ar-label">Current Limitation</div>
                  <div className="ar-value">
                    This is still an inferred signal view, not yet a true unmet-need registry backed by first-class unmet-need entities.
                  </div>
                </div>
                <div className="ar-row">
                  <div className="ar-label">Status</div>
                  <div className="ar-value">
                    Phase 1 intelligence view is now reading the live referral stream and surfacing likely unmet-need pressure.
                  </div>
                </div>
              </div>
            </div>
