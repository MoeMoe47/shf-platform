...
const ANALYST_ECOSYSTEM_LOCKED_BACKBONES = SHS_V1_REQUIRED_BACKBONES.map((id) => ({
  id,
  status: "locked",
  modelActive: true,
  surfaceActive: true,
...
function EcosystemReadinessAnalystMemoBridge() {
  const model = buildEcosystemReadinessModel({
    source: "analyst_memo_ecosystem_readiness",
    backbones: ANALYST_ECOSYSTEM_LOCKED_BACKBONES,
  ...
  return (
    <article
      className={[
        ...
        "ecosystem-readiness-analyst-memo-bridge",
        ecosystemReadinessStatusClass(model.readinessStatus),
      ].join(" ")}
      data-export-bridge="ecosystem-readiness-analyst-memo"
    >
      <div className="reporting-command__card-top">
        <div>
          <p className="reporting-command__card-kicker">V1 Readiness Intelligence</p>
          <h3 className="reporting-command__card-title" style={{ fontSize: 22 }}>
            {model.readinessLabel}
          </h3>
        </div>
        <StatusBadge status={model.readinessStatus === "v1_locked" ? "ready" : "pending"} />
      </div>

      <DecisionBand
        summary={`V1 readiness ${model.readinessScore}% | Locked ${model.summary.lockedBackbones}/${model.summary.requiredBackbones} | Drift ${model.summary.driftClean ? "clean" : "findings"} | Backend ${model.summary.backendValidationPassed ? "passed" : "review"}`}
      />
      ...
      >
        {model.rows.slice(0, 6).map((row) => (
          <MemoSectionCard
            key={row.id}
            title={row.label}
            detail={`${row.readinessLabel}. Readiness score ${row.readinessScore}%. Model ${row.checks.modelActive ? "active" : "missing"}. Surface ${row.checks.surfaceActive ? "active" : "missing"}. Audit ${row.checks.auditPassed ? "passed" : "needs review"}.`}
          />
        ))}
      </div>
      ...
      >
        <strong style={{ display: "block", marginBottom: 5 }}>Analyst readiness note</strong>
        <span>
          This readiness bridge gives analysts a single V1 proof layer showing which SHS backbones are locked, whether validation passed, and whether open launch risks remain.
        </span>
      </div>
    </article>
  );
...
export default function AnalystMemoExportPanel() {
  ...
  return (
    <section className="reporting-command__card" style={{ gap: 18 }}>
      ...
      <EcosystemReadinessAnalystMemoBridge />
