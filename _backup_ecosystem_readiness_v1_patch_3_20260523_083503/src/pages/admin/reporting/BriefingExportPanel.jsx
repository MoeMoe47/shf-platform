...
const BRIEFING_ECOSYSTEM_LOCKED_BACKBONES = SHS_V1_REQUIRED_BACKBONES.map((id) => ({
  id,
  status: "locked",
  modelActive: true,
  surfaceActive: true,
...
function EcosystemReadinessBriefingBridge() {
  const model = buildEcosystemReadinessModel({
    source: "briefing_export_ecosystem_readiness",
    backbones: BRIEFING_ECOSYSTEM_LOCKED_BACKBONES,
  ...
  return (
    <article
      className={[
        ...
        "ecosystem-readiness-briefing-bridge",
        ecosystemReadinessStatusClass(model.readinessStatus),
      ].join(" ")}
      data-export-bridge="ecosystem-readiness-briefing"
    >
      <div className="reporting-command__card-top">
        <div>
          <p className="reporting-command__card-kicker">SHS Ecosystem Readiness</p>
      ...
      >
        {model.rows.slice(0, 9).map((row) => (
          <article className="reporting-command__card" key={row.id}>
            <div className="reporting-command__card-top">
              <div>
                <p className="reporting-command__card-kicker">{row.readinessLabel}</p>
                <h4 className="reporting-command__card-title" style={{ fontSize: 17 }}>
                  {row.label}
                </h4>
              </div>
            </div>
            ...
            <p className="reporting-command__summary" style={{ fontSize: 13, lineHeight: 1.6 }}>
              Readiness: {row.readinessScore}% · Model: {row.checks.modelActive ? "Active" : "Missing"} · Surface: {row.checks.surfaceActive ? "Active" : "Missing"} · Audit: {row.checks.auditPassed ? "Passed" : "Review"}
            </p>
          </article>
        ))}
      </div>
...
export default function BriefingExportPanel() {
  ...
  return (
    <section className="reporting-command__card" style={{ gap: 16 }}>
      ...
      <EcosystemReadinessBriefingBridge />
