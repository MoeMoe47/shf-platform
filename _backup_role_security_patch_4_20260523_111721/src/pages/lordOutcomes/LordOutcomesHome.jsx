...
const SHS_ROLE_SECURITY_DEMO_RESULTS = SHS_ROLE_SECURITY_UX_REGISTRY.map((check) => ({
  id: check.id,
  status: "not_tested",
  finding: "",
  severity: "none",
  notes: "Pending role/security UX browser check.",
  testedAt: null,
}));

function RoleSecurityDashboardSurface() {
  const model = buildRoleSecurityUxModel({
    source: "lord_outcomes_home",
    results: SHS_ROLE_SECURITY_DEMO_RESULTS,
  });
    ...
    <section className={["lo-roleSecurity", roleSecurityUxStatusClass(model.roleSecurityStatus)].join(" ")}>
              ...
              <ul>
                {checks.slice(0, 5).map((check) => (
                  <li key={check.id}>
                    ...
                    <small>Expected: {check.expectedAccess}</small>
                  </li>
                ))}
              </ul>
            </article>
      ...
      <div className="lo-roleSecurity__decision">
        <div>
          ...
          <p>
            The role/security registry is active. Checks are ready for manual browser testing and later real status capture.
          </p>
        </div>
