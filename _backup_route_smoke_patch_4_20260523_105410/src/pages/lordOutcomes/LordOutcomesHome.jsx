...
const SHS_ROUTE_SMOKE_DEMO_RESULTS = SHS_ROUTE_SMOKE_REGISTRY.map((route) => ({
  id: route.id,
  status: "not_tested",
  httpStatus: null,
  consoleErrors: [],
  notes: "Pending manual browser smoke test.",
}));

function RouteSmokeDashboardSurface() {
  const model = buildRouteSmokeTestModel({
    source: "lord_outcomes_home",
    results: SHS_ROUTE_SMOKE_DEMO_RESULTS,
  });
    ...
    <section className={["lo-routeSmoke", routeSmokeStatusClass(model.routeSmokeStatus)].join(" ")}>
      ...
      <div className="lo-routeSmoke__decision">
        <div>
          ...
          <p>
            The route smoke registry is active. Routes are ready for manual browser testing and later automated status capture.
          </p>
        </div>
