...
export function registerIdentityRoutes(app: any) {
  ...
  app.post("/identity/memberships", requirePermission("membership.assign"), async (req: any, res: any) => {
    try {
      res.json(
        ok({
  ...
  app.patch("/identity/memberships/:id/revoke", requirePermission("membership.revoke"), async (req: any, res: any) => {
    res.json(
      ok({
