...
export default function IEPCommandCenterV2() {
  ...
  useEffect(() => {
    ...
    const actionCard = findCardByTitle(leftRail, /recommended\s+action/i);
    if (actionCard) {
      const actionTitle = actionCard.querySelector(".v2-action-title");
      const actionSub = actionCard.querySelector(".v2-action-sub");

      if (actionTitle) {
        actionTitle.textContent = `${selected.leftActionLabel} → ${selected.displayName.toUpperCase()}`;
      }
      if (actionSub) {
        actionSub.textContent = selected.leftSummary;
      }
    }
  }, [selectedCaseKey]);
      ...
      <main className="v2-main">
        ...
        <section className="v2-center-stage">
          <div className="v2-center-header">
            <div className="v2-center-header-main">
              <div className="v2-center-title">County Command Surface</div>
              <div className="v2-surface-search-wrap">
                <span className="v2-surface-search-icon" aria-hidden="true">⌕</span>
                <input
                  className="v2-surface-search-input"
                  type="text"
                  value={surfaceSearch}
                  onChange={(e) => setSurfaceSearch(e.target.value)}
                  placeholder="Search student, district, case, or alert"
                  aria-label="Search student, district, case, or alert"
                />
              </div>
            </div>
              ...
              <div className="v2-field-view">
                ...
                <div className="v2-map-surface">
                  <OhioCountyNeutralBase
                    ...
                    onCentroidsChange={setCountyCentroids}
                  />
                  <CountyInteractionLayer
                    county={activeCountyState || profile.label}
                    profile={profile}
                    countyCentroids={countyCentroids}
                    onSelectCounty={setActiveCountyState}
                    onOpenDetail={() => navigate(`/county/${activeCountyState || profile.label}`)}
                  />
                </div>
