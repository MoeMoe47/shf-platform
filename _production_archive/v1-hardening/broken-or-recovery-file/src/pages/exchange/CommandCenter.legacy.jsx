...
import { initAnalystListener } from "./analystListener";
import AnalystPredictionCard from "./AnalystPredictionCard";
import { buildAnalystNarrative } from "./buildAnalystNarrative";
import CommandCenterReportActions from "./CommandCenterReportActions";
import { useCommandCenterReportActions } from "./useCommandCenterReportActions";
import { buildPredictionLayer } from "./buildPredictionLayer";
import CommandCenterStats from "./CommandCenterStats";
import DecisionSignalCard from "./DecisionSignalCard";
import { buildDecisionSignal } from "./buildDecisionSignal";
import { buildComputedAgentContext } from "./buildComputedAgentContext";
import { buildActiveAgentContext } from "./buildActiveAgentContext";
import SimulationPanel from "./sim/SimulationPanel";
import { buildSimulationViewData } from "./sim/buildSimulationViewData";
import { buildSimulationEngineContext } from "./sim/buildSimulationEngineContext";
...
function RightDecisionPanel({ rightPanel, activeCase, selectedPanel, setSelectedPanel, setRecommendation, setSystemStatus, setTimelineStep, timelineStep, onExplainFocus }) {
...
function RecommendedNextMove({ recommendation, activeCase, predictive }) {
  return (
    <div style={{
        ...panel,
        padding: "12px 16px",
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ color: "#edf5fe", fontSize: 15, fontWeight: 800, marginBottom: 4, letterSpacing: "-0.01em" }}>
          {recommendation?.action || predictive?.recommendedAction || "assign_verifier"}
        </div>
      ...
      <div style={{
        ...
        {Math.round(recommendation?.confidence || predictive?.confidence || 0)}% confidence
      </div>
    </div>
  );
}
...
/* Phase 2 removed duplicate prediction engine */
...
function AIAnalystDrawer({ isOpen, onClose, agentContext, agentAudience, analystFocus, predictionLayer }) {
...
const predictive = buildPredictiveOutlook(safeAgentContext, agentAudience);
  const drawerNarrative = buildAnalystNarrative({
    predictionLayer: predictive,
    activeCase: safeAgentContext?.activeCase,
    systemStatus: safeAgentContext?.systemStatus,
    timelineStep: safeAgentContext?.timelineStep,
    recommendation: safeAgentContext?.recommendation || {},
    agentContext: safeAgentContext,
  });
...
export default function CommandCenter({ onShellStateChange } = {}) {
  React.useEffect(() => {
    console.log("INIT LISTENER RUNNING");

    const handler = (e) => {
      console.log("SYSTEM RECEIVED ACTION:", e.detail?.type);
    };
  ...
  React.useEffect(() => {
    initAnalystListener({ setRecommendation, setTimelineStep });
  }, [setRecommendation, setTimelineStep]);
  ...
  // 🔥 SHF EVENT BRIDGE (sandbox → real state)
  React.useEffect(() => {
    const handler = (e) => {
      const action = e.detail?.type;

      console.log("🔥 COMMAND CENTER RECEIVED:", action);
    ...
    window.addEventListener("shf:ai_action", handler);
    return () => window.removeEventListener("shf:ai_action", handler);
  }, []);

  const simulationEngineContext = buildSimulationEngineContext({
    enabled: true,
    tickMs: 5000,
    agentContext,
    recommendation,
  ...
  const simulationFrame = useExchangeSimulation(simulationEngineContext);
  const simRecommendation = simulationFrame?.recommendation || null;
  const simAnalyst = simulationFrame?.analystSummary || null;
  const simulationViewData = buildSimulationViewData({
    simulationFrame,
    simAnalyst,
  });
...
const computedAgentContext = buildComputedAgentContext({
    agentContext,
    liveRecommendation,
    recommendation,
    liveTimelineStep,
  ...
  const activeAgentContext = buildActiveAgentContext({
    liveAgentContext,
  ...
  } = useCommandCenterReportActions({
    agentContext: activeAgentContext,
    proofMetrics: data?.proofMetrics,
  });

  const predictive = buildPredictionLayer(
    activeAgentContext,
    agentAudience
  );
  ...
  }, [predictive, narrative, onShellStateChange]);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <FeatureGate feature="decisionSignal">
        <DecisionSignalCard
          label={decisionSignal.label}
          stateLabel={decisionSignal.stateLabel}
          tone={decisionSignal.tone}
          confidence={decisionSignal.confidence}
          recommendation={decisionSignal.recommendation}
        />
        </FeatureGate>

        <div className="alive-recommendation">

          <FeatureGate feature="reportActions">
          <CommandCenterReportActions
            onGenerate={handleGenerateBrief}
            onOpenLatest={handleOpenLatestBrief}
          />
          </FeatureGate>
        ...
        <FeatureGate feature="simulationPanel">
        <SimulationPanel
          simulation={simulationViewData}
        />
        </FeatureGate>
