...
import { askGovernmentAssuranceAssistant, generateGovernmentAssuranceReport, getAiDelegations } from "../../services/government-assurance-client";
...
export default function GovernmentAssurancePhase8B({ dashboard = null }) {
  ...
  async function generate(event) {
    ...
    try { setReport(await generateGovernmentAssuranceReport({ reportType, subjectReference: subjectReference || undefined })); }
    catch (requestError) { setError(requestError.message); }
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `${report.report.reportType.toLowerCase()}-${report.artifact?.artifact_id || "report"}.json`; link.click(); URL.revokeObjectURL(url);
  }
  ...
  return <div className="gpa-grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", alignItems: "start" }}>
    <section style={{ border: "1px solid #d9e2ec", padding: 16 }} aria-labelledby="gpa-assistant-heading">
      <h2 id="gpa-assistant-heading">Governed GPA Assistant</h2>
    ...
    <section style={{ border: "1px solid #d9e2ec", padding: 16 }} aria-labelledby="gpa-reports-heading">
      ...
      <form onSubmit={generate} style={{ display: "grid", gap: 10 }}>
        ...
        <label htmlFor="gpa-report-subject">Optional canonical subject reference</label>
