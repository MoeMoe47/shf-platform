...
export class ProjectService {
  constructor(private dbQuery: typeof query=query, private transaction=withTransaction, private outbox=new IntegrationOutboxRepo()) {}
  ...
  async review(actor:any,submissionId:string,status:string) { if(!hasPermission(actor?.permissions,SHS_SECURITY_PERMISSIONS.PROJECT_SUBMISSION_REVIEW)) throw new Error("project_submission_review_required"); const s=scope(actor); if(!["ACCEPTED","NEEDS_REVISION"].includes(status)) throw new Error("invalid_project_review"); const r=await this.dbQuery("UPDATE project_submissions SET status=$2 WHERE submission_id=$1 AND organization_id=$3 AND tenant_id=$4 RETURNING *",[submissionId,status,s.organizationId,s.tenantId]); if(!r.rows[0]) throw new Error("submission_not_found"); if(status === "ACCEPTED") await this.outbox.enqueue({ producer_id:"curriculum.project", event_type:"project.submission.reviewed", schema_version:"1.0", subject_type:"project_submission", subject_id:r.rows[0].submission_id, organization_id:s.organizationId, originating_actor_id:s.userId, originating_actor_type:"user", tenant_id:s.tenantId, occurred_at:new Date().toISOString(), idempotency_key:`project.submission.reviewed:${r.rows[0].submission_id}:${status}`, correlation_id:`project:${r.rows[0].submission_id}`, payload:{ source_record_id:r.rows[0].submission_id, review_status:status, reviewer_user_id:s.userId }, destination:"shs-verified-evidence" }); return r.rows[0]; }
