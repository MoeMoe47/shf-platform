...
# === ALLOWLIST_REPLAY (Top-1%): permit deterministic replay ingestion ===
try:
    _pg_path = request.scope.get("path", "")
    _pg_method = (request.scope.get("method") or "").upper()
except Exception:
    _pg_path = ""
    _pg_method = ""

# Allow POST/GET under /api/funding/replay/*
if isinstance(_pg_path, str) and _pg_path.startswith("/api/funding/replay/"):
    # replay endpoints are audit infrastructure, not funding execution
    return await call_next(request)
# === END ALLOWLIST_REPLAY ===
