...
def build_sdk():
    """
    Gold-standard partner quickstart. Keep stable.
    """
    return {
        "schema_version": "AIM_SDK_V1",
        "generated_at": _now_iso(),
        "title": "SHF Funding API — Partner SDK Quickstart",
        "overview": [
            "This is a read-only integration surface for partners to self-integrate.",
            "Core flow: capabilities → schemas/examples/postman → discovery → fetch by id → simulate."
        ],
        "base_url": "http://127.0.0.1:8001",
        "endpoints": {
            "health": "/api/funding/health",
            "capabilities": "/api/funding/capabilities",
            "schemas": "/api/funding/schemas",
            "examples": "/api/funding/examples",
            "postman": "/api/funding/postman",
            "discovery": "/api/funding/discovery",
            "rulesets": "/api/funding/rulesets",
            "lock_triggers": "/api/funding/lock-triggers",
            "simulate": "/api/funding/simulate",
        },
        "curl": [
            ...
            "curl -sS http://127.0.0.1:8001/api/funding/schemas | python3 -m json.tool",
        ...
        "simulate_curl": [
            "curl -sS http://127.0.0.1:8001/api/funding/simulate \\",
            "  -H 'content-type: application/json' \\",
            "  -d '{\"ruleset_id\":\"rs_2026Q2_workforce_v1_0\",\"cohort\":{\"participants_enrolled\":24,\"participants_completed\":19,\"attendance_rate\":0.87,\"job_placement_count\":11,\"verified_placement_rate\":0.73,\"certification_earned_count\":8,\"retention_3m_rate\":0.62,\"retention_6m_rate\":0.48,\"participant_opportunity_index\":0.60},\"benchmarks\":{\"cost_per_job_placement_percentile\":0.44},\"watchtower\":{\"signals\":[]}}' \\",
            "  | python3 -m json.tool",
        ],
        "python_requests_example": [
            "import requests",
            "BASE='http://127.0.0.1:8001'",
            "payload={...}  # use simulate_example_request",
            "r=requests.post(f'{BASE}/api/funding/simulate', json=payload, timeout=30)",
            "r.raise_for_status()",
            "print(r.json())",
        ],
        "notes": [
            "These docs are stable partner contracts; the full truth remains OpenAPI at /openapi.json.",
            "All endpoints are read-only except /api/funding/simulate (compute-only).",
            "No secrets should ever be exposed by this surface."
        ]
    }
