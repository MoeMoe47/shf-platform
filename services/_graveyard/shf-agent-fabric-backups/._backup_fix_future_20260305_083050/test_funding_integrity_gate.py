...
def _post(path: str, payload: Dict[str, Any], timeout: float = 15.0) -> Dict[str, Any]:
    r = client().post(f"{BASE}{path}", json=payload, timeout=timeout)
    r.raise_for_status()
    return r.json()
...
def test_funding_surface_is_compute_only():
    ...
    """
    r = client().post(f"{BASE}/api/funding/sdk", timeout=10)
    assert r.status_code == 405

    # simulate is allowed but may reject missing body
    r2 = client().post(f"{BASE}/api/funding/simulate", timeout=10)
