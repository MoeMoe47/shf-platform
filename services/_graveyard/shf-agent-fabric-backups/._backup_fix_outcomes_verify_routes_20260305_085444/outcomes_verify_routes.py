...
from fabric.outcomes.verify_store import get_credit_by_submission, get_intent_by_credit, get_funding_event_link, link_funding_event
from fabric.funding.journal.outcome_bridge import make_event_payload, append_event, compute_event_hash, find_event
...
def verify(submission_id: str) -> Dict[str, Any]:
    ...
    # --- Funding Journal Bridge (append-only, replayable) ---
    # Create or reuse funding event link for this submission
...
def chain(submission_id: str) -> Dict[str, Any]:
    ...
    if funding_event and funding_event.get("payload"):
        # recompute funding event hash for integrity check
