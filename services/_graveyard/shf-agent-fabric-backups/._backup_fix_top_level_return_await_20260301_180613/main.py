...
# === FUNDING POLICY GUARD (STRICT MODE) ===
from fastapi import Request
from fastapi.responses import JSONResponse
from starlette.middleware.base import BaseHTTPMiddleware

class FundingPolicyGuardMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if path.startswith("/api/funding/"):
            ...
            qp = str(request.url.query).lower()

            if any(x in qp for x in ["write","mutate","update","delete"]):
                return JSONResponse(
                    status_code=403,
                    content={
        ...
        return await call_next(request)
            ...
            # Everything else under /api/funding/* is blocked
            return JSONResponse(
                status_code=405,
                content={
        return await call_next(request)
...
@app.middleware("http")
async def funding_policy_guard(request, call_next):
    """
    Strict Funding Policy Guard:
    - Under /api/funding/* allow only:
      - GET/HEAD/OPTIONS for any funding endpoint
      - POST only for /api/funding/simulate (compute-only)
    """
    path = request.url.path
    method = request.method.upper()

    if path.startswith("/api/funding/"):
        # Always allow reads / discovery
        if method in ("GET", "HEAD", "OPTIONS"):
            return await call_next(request)

        # Allow only compute-only simulate
        if method == "POST" and path == "/api/funding/simulate":
            return await call_next(request)
...
app.add_middleware(FundingPolicyGuardMiddleware)
