...
def verify_proof_signature(submission_id: str, payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
    """
    Verify deterministic signature integrity.

    If caller provides {"signature": "..."} we verify that.
    Otherwise, we try to read signature from the proof bundle (if present).
    """
    payload = payload or {}

    proof = proof_bundle(submission_id)

    # signature provided by caller OR embedded in proof bundle
    provided_sig = (
        payload.get("signature")
        or proof.get("signature")
        or (proof.get("proof_bundle") or {}).get("signature")
    )

    # choose what to sign: prefer inner proof_bundle if present
    sign_target = proof.get("proof_bundle") if isinstance(proof.get("proof_bundle"), dict) else proof

    # remove volatile + signature fields before signing
    proof_canonical = {
        k: v for k, v in sign_target.items()
        if k not in ("generated_at", "signature", "signature_algorithm", "algorithm")
    }

    signed = _sign_proof(proof_canonical)
    expected_sig = signed["signature"] if isinstance(signed, dict) else str(signed)
