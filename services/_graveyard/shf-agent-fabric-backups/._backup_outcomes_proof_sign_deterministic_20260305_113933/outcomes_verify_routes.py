...
def sign_proof(submission_id: str) -> Dict[str, Any]:
    """
    Returns a signed proof bundle for audit/verification.
    Signature = HMAC-SHA256 over canonical JSON (sorted keys).
    """
    from datetime import datetime, timezone

    # Reuse local proof endpoint function (added earlier)
    proof_bundle = proof(submission_id)

    key = os.environ.get("SHF_PROOF_SIGNING_KEY", "dev_signing_key_do_not_use_in_prod")
    payload = json.dumps(proof_bundle, sort_keys=True, separators=(",", ":")).encode("utf-8")
