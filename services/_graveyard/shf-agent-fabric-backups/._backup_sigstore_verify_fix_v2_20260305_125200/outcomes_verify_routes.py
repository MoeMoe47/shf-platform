...
def verify_proof_signature(submission_id: str, body: dict | None = Body(default=None), payload: Dict[str, Any] | None = None) -> Dict[str, Any]:

    """
    Verify deterministic signature integrity (Option A).
    Primary: compare stored signature (written at /sign) vs recomputed expected.
    Fallback: if client provides {"signature": "..."} in body, use it.
    """
    proof = proof_bundle(submission_id)
    ...
    # Fallback: body signature (for CLI + tests)
    if isinstance(body, dict):
        provided_sig = body.get("signature") or body.get("provided_signature")

    # Primary: stored signature
    if not provided_sig:
        stored = proof_signature_store.get_latest_signature(submission_id)
        if isinstance(stored, dict):
            provided_sig = stored.get("signature")
    else:
        stored = proof_signature_store.get_latest_signature(submission_id)  # still report store meta if present
    ...
    return {
        ...
        "store_schema_version": (stored.get("schema_version") if isinstance(stored, dict) else None),
        "stored_proof_sha256": (stored.get("proof_sha256") if isinstance(stored, dict) else None),
    }


# === SIGSTORE (Option A) ===
_SIGSTORE_SCHEMA = "OUTCOMES_PROOF_SIGNATURE_STORE_V1"

def _sigstore_path() -> Path:
    # absolute + stable: services/shf-agent-fabric/var/outcomes_proof_signatures.jsonl
    here = Path(__file__).resolve()
    svc = here.parents[2]  # .../services/shf-agent-fabric
    return svc / "var" / "outcomes_proof_signatures.jsonl"

def _sigstore_append(*, submission_id: str, algorithm: str, signature: str, proof_sha256: str) -> None:
    path = _sigstore_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    rec = {
        "store_schema_version": _SIGSTORE_SCHEMA,
...
def _sigstore_latest(submission_id: str):
    path = _sigstore_path()
    if not path.exists():
        return None
    latest = None
