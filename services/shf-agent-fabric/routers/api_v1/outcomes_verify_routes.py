...
def sign_proof(submission_id: str) -> Dict[str, Any]:
    ...
    # Persist signature record (append-only) using the actual computed values.
    try:
        _sigstore_append(
            submission_id=submission_id,
...
def _sigstore_latest(submission_id: str) -> Optional[Dict[str, Any]]:
    """Return the latest stored proof signature for this submission, if any."""
    return proof_signature_store.get_latest_signature(submission_id)

@router.post("/proof/{submission_id}/verify-signature")
def verify_proof_signature(submission_id: str, body: VerifySignatureRequest = Body(default_factory=VerifySignatureRequest)) -> Dict[str, Any]:
    """
    ...
    stored_sig: Optional[str] = None
    store_schema_version: Optional[str] = None
    stored_proof_sha256: Optional[str] = None
    if isinstance(stored, dict):
        stored_sig = stored.get("signature") or stored.get("sig")
        store_schema_version = stored.get("store_schema_version") or stored.get("schema_version")
        stored_proof_sha256 = stored.get("stored_proof_sha256") or stored.get("proof_sha256")
    provided_signature = (getattr(body, "signature", None) or stored_sig)
    ...
    # SHF_VERIFY_SIGNATURE_RETURN_GUARD
    # Guarantee JSON response on all paths (some branches previously returned None).
    _L = locals()
    if _L.get('match', None) is None and _L.get('expected_signature', None) is not None:
        _L['match'] = (_L.get('provided_signature') == _L.get('expected_signature'))
    return {
        "ok": True,
        "submission_id": submission_id,
        "match": bool(_L.get('match')),
        "provided_signature": _L.get('provided_signature'),
        "expected_signature": _L.get('expected_signature') or _L.get('expected_sig'),
        "algorithm": _L.get('algorithm'),
        "store_schema_version": _L.get('store_schema_version'),
        "stored_proof_sha256": _L.get('stored_proof_sha256'),
    }
