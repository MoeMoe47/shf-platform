...
def _generate_report(report_definition_id: str, actor: Any, period_start: str, period_end: str, public: bool = False, correlation_id: str | None = None) -> Dict[str, Any]:
    ...
    metric = calculate_metric(definition["metric_ids"][0], actor.organization_id, period_start, period_end, actor, public=public, correlation_id=correlation_id)
    if public and metric.get("public_eligibility") is not True:
    ...
    return {
        ...
        "generated_at": metric["calculated_at"],
        "metric_results": [metric],
        "verification_status": metric["verification_status"],
        "public_eligibility": metric["public_eligibility"],
        "suppression_status": metric["suppression_status"],
        "definition_digest": metric["definition_digest"],
        "lineage_reference": {"source_claim_ids": metric["source_claim_ids"], "source_claim_versions": metric.get("source_claim_versions", []), "source_evidence_ids": metric["source_evidence_ids"]},
