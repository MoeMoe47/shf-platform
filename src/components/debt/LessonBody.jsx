// src/components/debt/LessonBody.jsx
...
/* ------------------------------------------------------------------
   MicroQuiz (same API as Civic/Credit)
------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------
   NotesModal — quick capture → Portfolio
------------------------------------------------------------------- */
...
export default function LessonBody({ lesson, nextId, onGoNext }) {
  ...
  // coach suggestion -> append with undo
  React.useEffect(() => {
    const onSuggest = (e) => {
  ...
  // legacy quiz total listener (kept for parity)
  React.useEffect(() => {
    const onQuiz = () => {
      try {
        const kTot = `${APP}:quiz:correctTotal`;
        const prev = Number(localStorage.getItem(kTot) || "0");
        const total = prev; // placeholder; your new MicroQuiz flow writes points directly
  ...
  const addToPortfolio = React.useCallback(() => {
    ...
    try {
      ...
      const artifact = {
        ...
        kind: "lesson",
        title: vLesson?.title || `Debt Lesson ${id}`,
        lessonId: id,
        createdAt: Date.now(),
        tags: ["debt", "lesson"],
        pathwayId: null,
    ...
    <div className="lesson-body" data-surface="soft">
      <div className="lesson-surface">
        <div className="sh-grid" style={{ gap: 12 }}>
          ...
          {/* Content (with inline quiz injection) */}
          <section ref={sectionRefs.current.content} data-section="content" className="sh-card" aria-labelledby="sec-content">
            ...
            <div className="sh-cardBody">
              ...
              {/* Legacy support: bottom quiz block */}
            ...
            <div className="sh-cardBody">
              ...
              <div className="sh-cardContent">
                ...
                <textarea
                  ...
                  placeholder={"What did you take away from this debt lesson? List steps you’ll take (snowball vs avalanche, extra payments, APR calls) 💸"}
                />
