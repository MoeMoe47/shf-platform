// src/components/treasury/LessonBody.jsx
    ...
    <div className="kb-overlay" onClick={(e)=>{ if (e.target === e.currentTarget) onClose?.(); }}>
      <div className="kb-card" role="dialog" aria-modal="true" aria-label="Add Note">
        ...
        <textarea
          ...
          placeholder="Capture a ledger insight, control idea, or budget note."
          value={text}
          onChange={e=>setText(e.target.value)}
        />
        <div className="sh-actionsRow" style={{ marginTop: 8 }}>
          <button className="sh-btn" onClick={() => { onSave?.(text.trim()); setText(""); onClose?.(); }} disabled={!text.trim()}>
            Save to Portfolio
          </button>
...
export default function LessonBody({ lesson, nextId, onGoNext }) {
  ...
  React.useEffect(() => { ping(`${APP}.lesson.view`, { id, title: vLesson?.title || "" }); }, [id, vLesson?.title]);
  ...
  const onComplete = React.useCallback(() => {
    ...
    try {
      ...
      totalCompleted = k
        .filter(key => key.startsWith(`${APP}:lesson:`) && key.endsWith(":complete") && localStorage.getItem(key) === "1")
        .length;
    } catch {}
  ...
  const addToPortfolio = React.useCallback(() => {
    ...
    try {
      ...
      const artifact = {
        ...
        kind: "lesson",
        title: vLesson?.title || `Treasury Lesson ${id}`,
        lessonId: id,
        createdAt: Date.now(),
        tags: ["treasury", "lesson"],
        pathwayId: null,
        ...
        <div className="sh-grid" style={{ gap: 12 }}>
          ...
          <section ref={sectionRefs.current.content} data-section="content" className="sh-card" aria-labelledby="sec-content">
            ...
            <div className="sh-cardBody">
              ...
              {/* Bottom quiz (legacy array format) */}
            ...
            <div className="sh-cardBody">
              ...
              <div className="sh-cardContent">
                ...
                <textarea
                  ...
                  placeholder={"What allocation or control will you implement next (budget caps, approval rules, vendor limits, audit tags, reserve targets)? 💰🧾"}
                />
