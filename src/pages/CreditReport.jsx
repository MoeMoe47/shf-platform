...
export default function CreditReport() {
  ...
  // Avg age (yrs) higher is better — rough bands
  ...
  return (
    <div
      className="page pad"
      data-scope="credit-report"
      style={{ display: "grid", gap: 16 }}
    >
      <header
        className="card card--pad"
        role="group"
        aria-label="Credit overview"
      >
        <div
          style={{
            display: "flex",
        ...
        >
          <h1 style={{ margin: 0 }}>FICO® Score</h1>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
            <button
              ...
              onClick={onExport}
              aria-label="Export credit report JSON"
            >
              Export JSON
            </button>
            <button
              ...
              onClick={() => {
                if (confirm("Clear local credit history? (Demo only)"))
                  clearLocal();
              }}
            >
              Reset (demo)
        ...
        >
          ...
          <KpiCard
            ...
            value={`${Number.isFinite(kpis.avgAgeYears) ? kpis.avgAgeYears.toFixed(1) : "—"} yrs`}
      ...
      <div
        className="grid"
        style={{ display: "grid", gap: 16, gridTemplateColumns: "2fr 1fr" }}
      >
        <section className="card card--pad" aria-labelledby="history-h">
          <h2 id="history-h" style={{ margin: 0, fontSize: 18 }}>
            Recent changes
          </h2>
          <ul
              ...
              >
                <span style={{ opacity: 0.85 }}>
                  ...
                  {e.reason && e.reason.startsWith("cap") && (
                    <em style={{ marginLeft: 6, color: "var(--ink-soft)" }}>
                      ({e.reason})
                    </em>
                  )}
                </span>
                <strong
                  style={{
                    color: (e.delta ?? 0) >= 0 ? "var(--ok)" : "var(--danger)",
                  }}
                >
                  {(e.delta ?? 0) > 0 ? "+" : ""}
                  {e.delta ?? 0}
      ...
      <section className="card card--pad" aria-labelledby="movers-h">
        <h2 id="movers-h" style={{ margin: 0, fontSize: 18 }}>
      ...
      <footer className="card card--pad" style={{ color: "var(--ink-soft)" }}>
        FICO® Score shown here is educational/simulated from SHF activity; it
        is <strong>not</strong> a consumer credit disclosure.
      </footer>
    </div>
  );
}
