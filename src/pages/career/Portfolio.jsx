...
/* Washed KPI Attendance */
import AttendanceCard from "@/components/AttendanceCard.jsx";

/* Imported for future washed KPIs (ok to keep even if unused right now) */
...
function AvatarUploader({ storageKey = "sh_profile_photo", circle = true }) {
  ...
  async function handleFiles(list) {
    ...
    try {
      ...
      // Credit: profile completion (photo)
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button
                  ...
                  className="sh-btn"
                  onClick={pick}
                  style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid var(--line,#e5e7eb)", background: "#fff" }}
                >
                  Upload
                </button>
                {dataUrl && (
...
function MockInterviewCard({
  ...
  zoomHref = "https://zoom.us/j/123456789" // <-- swap with your live link
}) {
  ...
  const [answers, setAnswers] = useState(state.answers ?? {}); // { qid: { rating, notes } }
  ...
  function exportJSON() {
    ...
    // Credit: mock interview reflection exported
    earn({ action: "mock_interview.export", rewards: { wheat: 2 }, scoreDelta: 4 });
  }

  const scoreAvg = avg(
    Object.values(answers).map((r) => Number(r?.rating ?? 0)).filter(Boolean)
  );
