...
export default function Proposals() {
  ...
  const canSubmit =
    title.trim().length >= 4 && rationale.trim().length >= 10;
  ...
  const submit = (e) => {
    ...
    try {
      ...
      if (
        !localStorage.getItem(FLAG_FIRST_SUBMIT) &&
        !badges.includes("policy-author")
      ) {
        try {
      ...
      logWallet({
        note: `Proposal submitted: ${item.title}`,
        delta: +20,
      }); // capped to last 250
  ...
  const onVote = (id, dir) => {
    ...
    if (idx >= 0) {
      ...
      if (transitioned(oldStatus, newStatus)) {
        ...
        if (newStatus === "passed" && p.authorId === "local:user") {
          ...
          logWallet({
            note: `Your proposal passed: ${p.title}`,
            delta: +15,
          });
        }
      }
    ...
    if (!localStorage.getItem(FLAG_FIRST_VOTE)) {
      ...
      try {
        localStorage.setItem(FLAG_FIRST_VOTE, "1");
      } catch {}
    ...
    if (nextVote === +1)
      logWallet({ note: "Proposal upvoted", delta: +1 });
    if (nextVote === -1)
      logWallet({ note: "Proposal downvoted", delta: +1 });
  ...
  const onDeleteProposal = (id) => {
    ...
    toast("Proposal deleted.", {
      ...
      action: {
        ...
        onClick: () => {
          if (
            undoRef.current?.type === "deleteProposal" &&
            undoRef.current.proposal
          ) {
            const restored = [
              undoRef.current.proposal,
              ...readJSON(KEY_PROPOSALS, []),
            ];
  ...
  return (
    <section className="crb-main" aria-labelledby="pr-title">
      <header className="db-head">
        <div>
          <h1 id="pr-title" className="db-title">
            Proposals
          </h1>
        ...
        >
          <StorageSoftReset
            ...
            label="Fix storage"
            onDone={() =>
              toast("Storage reset for proposals.", { type: "info" })
            }
          />
        </div>
      </header>
      ...
      >
        ...
        >
          When you finish a proposal, log your work so it shows up
          in the grant story under ESSA / Civics.
        </p>
        <MissionLogButtons
  ...
  const seeded = [
    {
      ...
      impact:
        "Year 1: $150k grant; volunteer mentors; in-kind equipment.",
      score: 2,
      status: "open",
      statusAt: null,
      authorId: "local:user",
