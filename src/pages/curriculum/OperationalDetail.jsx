...
export default function OperationalDetail({ kind }) {
  ...
  return <main className="ops-page"><header className="ops-header"><div><p className="ops-eyebrow">Assignment detail</p><h1>{assignment.title}</h1><p>{assignment.assignment_type} · Due {new Date(assignment.due_at).toLocaleDateString()} · {assignment.status}</p></div><div><Link className="ops-secondary" to={`/studio/assignments/${encodeURIComponent(assignment.assignment_id)}/progress`}>Studio progress</Link> <Link className="ops-secondary" to="/curriculum/instructor/operations">Back to workspace</Link></div></header><div className="ops-grid"><section className="ops-panel"><h2>Canonical scope</h2><dl><dt>Organization</dt><dd>{assignment.organization_id}</dd><dt>Cohort</dt><dd>{assignment.cohort_name || "Organization"}</dd><dt>Release</dt><dd>{assignment.curriculum_release_id || "NOT_AVAILABLE"}</dd><dt>Completion Policy</dt><dd>{assignment.completion_policy_id || "NOT_AVAILABLE"}</dd></dl></section><DetailList title="Targets" items={data.targets} label={(x) => x.full_name || x.target_type} /><DetailList title="Canonical activity" items={data.truthFacts} label={(x) => x.fact_type} /></div></main>;
}
