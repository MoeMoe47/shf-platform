...
export default function DebtAvalanche() {
  ...
  const ordered = [...debts].sort((a, b) => b.apr - a.apr);
  const totalMin = debts.reduce((s, d) => s + d.min, 0);

  return (
    <div className="page pad" data-page="debt-avalanche">
      <header className="card card--pad">
        <h1 style={{ margin: 0 }}>Avalanche Calculator</h1>
        <p style={{ margin: "6px 0 0", color: "var(--ink-soft)" }}>
          Pays off highest APR first to minimize interest.
        </p>
      </header>
