...
function StageReferralCard({ item, index, compact = false, onVerifyForReport }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className={`rt-verifyInlineChip ${verifiedForReport ? "is-verified" : "is-unverified"}`}>
          {verifiedForReport ? "REPORT READY" : "NOT REPORT READY"}
        </span>
        <span className="rt-verifyActionText">
          <span>{verifiedForReport ? "REPORT READY" : "NOT REPORT READY"}</span>
        <strong>{verifiedForReport ? "Report Ready ✓" : "Mark Report Ready →"}</strong>
        {!verifiedForReport ? <em>Creates proof for reports</em> : <em>Ready for Hub Reports</em>}
        </span>
      </button>
...
export default function ReferralLifecycleView() {
  ...
  function handleVerifyForReport(itemOrId, index = 0) {
    ...
    try {
      verifyTruthSpineRecord(resolvedEntityId, {
      ...
      console.warn("[Truth Spine Engine] Verify for report failed", error);
      setVerifyNotice("Verification failed. Check Truth Spine record.");
      return;
    }
    ...
    setVerifyNotice("Referral marked report-ready. Hub Reports can now use this record.");
