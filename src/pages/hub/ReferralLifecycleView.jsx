...
function LifecycleReferralSignal({ item }) {
  const signal = buildLifecycleReferralSignal(item);

  return (
    <div className="rt-lifecycleSignalBox">
      <div
        className={[
          "rt-lifecycleSignal",
          lifecycleSignalClass(signal.tone),
        ].join(" ")}
        title={signal.stageMeaning}
      >
      ...
      {signal.blockers.length ? (
        ...
        <small>clear_for_next_stage</small>
      )}
    </div>
  );
}

function LifecycleStageSignal({ title, items = [] }) {
  const signal = buildLifecycleStageSignal(title, items);

  return (
    <div className="rt-stageSignalWrap">
      <span
        className={[
          "rt-stageSignal",
          lifecycleSignalClass(signal.tone),
        ].join(" ")}
        title={signal.nextAction}
      >
        {signal.label}
      </span>
      <small>
        {signal.blockedCount} blocked · {signal.reviewCount} review · {signal.reportableCount} reportable
      </small>
    </div>
  );
}
...
function StageReferralCard({ item, index, compact = false }) {
  ...
  return (
    ...
    >
      ...
      <LifecycleReferralSignal item={item} />
...
function StageColumn({ title, count, children, tone = "blue", items = [] }) {
  return (
    <section className={`rt-stage rt-stage--${tone}`}>
      ...
      <LifecycleStageSignal title={title} items={items} />
