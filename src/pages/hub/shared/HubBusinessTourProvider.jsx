...
function readGuidedWorkflowIntent() {
  if (typeof window === "undefined") return null;

  try {
    const raw = window.sessionStorage.getItem("shsHubGuidedWorkflowIntent");
    if (!raw) return null;
...
function shouldAutoStartHubTour() {
  if (typeof window === "undefined") return false;

  try {
    const hash = window.location.hash || "";
    const queryText = hash.includes("?") ? hash.split("?")[1] : "";
    const params = new URLSearchParams(queryText);

    return (
      params.get("tour") === "start" ||
      params.get("guided") === "true" ||
      Boolean(readGuidedWorkflowIntent()?.startTour)
    );
  } catch {
    return Boolean(readGuidedWorkflowIntent()?.startTour);
  }
}

function clearGuidedWorkflowIntent() {
  if (typeof window === "undefined") return;

  try {
    window.sessionStorage.removeItem("shsHubGuidedWorkflowIntent");
  } catch {
    // no-op
  }
}
  ...
  return (
    <div className="hubTour-root" data-hub-tour-active="true">
      ...
      >
        ...
        {guidedIntent ? (
          <div className="hubTour-guidedIntent">
            <span>Guided workflow context</span>
            <strong>{guidedIntent.page || guidedIntent.goalId || "Recommended workflow"}</strong>
            {guidedIntent.reason ? <p>{guidedIntent.reason}</p> : null}
            {guidedIntent.next ? (
              <small>Next: {guidedIntent.next}</small>
            ) : null}
          </div>
        ) : null}
...
function LegacyHubTour({ pageKey, children, steps }) {
  ...
  React.useEffect(() => {
    ...
    const intent = readGuidedWorkflowIntent();
    ...
    const startWhenTargetExists = () => {
      if (cancelled) return;

      attempts += 1;

      const firstVisibleTarget = steps.find((step) => findTarget(step?.target));
      const shouldForceStart = attempts >= 12;

      if (firstVisibleTarget || shouldForceStart) {
        setCurrentIndex(0);
        setIsActive(true);
        clearGuidedWorkflowIntent();

        try {
          const hash = window.location.hash || "";
          if (hash.includes("?")) {
            const [baseHash] = hash.split("?");
            window.history.replaceState(null, "", `${window.location.pathname}${window.location.search}${baseHash}`);
          }
        } catch {
          // no-op
        }
      ...
      timer = window.setTimeout(startWhenTargetExists, 250);
    };

    timer = window.setTimeout(startWhenTargetExists, 650);
