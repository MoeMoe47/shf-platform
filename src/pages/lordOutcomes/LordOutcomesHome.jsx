...
import { SHS_ROUTE_SMOKE_GROUPS, SHS_ROUTE_SMOKE_REGISTRY, buildRouteSmokeTestModel, buildRouteSmokeRows, routeSmokeStatusClass } from "../../shared/hardening/routeSmokeTestRegistry.js";
import routeSmokeTestResults from "../../shared/hardening/routeSmokeTestResults.json";
import { SHS_ROLE_SECURITY_GROUPS, SHS_ROLE_SECURITY_UX_REGISTRY, buildRoleSecurityUxModel, buildRoleSecurityUxRows, roleSecurityUxStatusClass } from "../../shared/hardening/roleSecurityUxRegistry.js";
import roleSecurityUxResults from "../../shared/hardening/roleSecurityUxResults.json";
import { SHS_MANUAL_BROWSER_SIGNOFF_GROUPS, SHS_MANUAL_BROWSER_SIGNOFF_REGISTRY, buildManualBrowserSignoffModel, buildManualBrowserSignoffRows, manualBrowserSignoffStatusClass } from "../../shared/hardening/manualBrowserSignoffRegistry.js";
import manualBrowserSignoffResults from "../../shared/hardening/manualBrowserSignoffResults.json";
...
function ManualBrowserSignoffDashboardSurface() {
  const snapshot = manualBrowserSignoffResults || {};
  const model = snapshot.model || buildManualBrowserSignoffModel({
    source: "lord_outcomes_home_fallback",
    results: [],
  });

  const rows = buildManualBrowserSignoffRows(model);
  const generatedAt = snapshot.generatedAt
    ? new Date(snapshot.generatedAt).toLocaleString()
    : "Not generated";

  const groupedItems = SHS_MANUAL_BROWSER_SIGNOFF_REGISTRY.reduce((groups, item) => {
    groups[item.group] ||= [];
  ...
  return (
    <section className={["lo-manualSignoff", manualBrowserSignoffStatusClass(model.signoffStatus)].join(" ")}>
      <div className="lo-manualSignoff__header">
        <div>
          <p className="lo-manualSignoff__eyebrow">Post-V1 Manual Browser Signoff</p>
          <h2>{model.signoffLabel}</h2>
          <span>{model.recommendedNextAction}</span>
        </div>

        <div className="lo-manualSignoff__score">
          <small>Items Tested</small>
          <strong>{model.summary.testedItems}/{model.summary.totalItems}</strong>
          <em>{model.summary.notTestedItems} not tested</em>
        </div>
      </div>

      <div className="lo-manualSignoff__snapshotMeta">
        <div>
          <strong>Manual browser signoff snapshot</strong>
          <span>Generated: {generatedAt}</span>
        </div>
        <div>
          <strong>Mode</strong>
          <span>{snapshot.mode || "No snapshot mode found"}</span>
        </div>
      </div>

      <div className="lo-manualSignoff__grid">
      ...
      <div className="lo-manualSignoff__groups">
        <div className="lo-manualSignoff__groupsHeader">
          <strong>Manual browser signoff groups</strong>
          <span>Public pages, admin workspace, Lord of Outcomes, Hub, Reporting, Exchange, Auth/Security, and responsive safety.</span>
        </div>

        <div className="lo-manualSignoff__groupList">
          {Object.entries(groupedItems).map(([group, items]) => (
            <article key={group}>
              <div>
                <span>{SHS_MANUAL_BROWSER_SIGNOFF_GROUPS[group] || group}</span>
                <strong>{items.length} items</strong>
                <em>{items.filter((item) => item.priority === 1).length} priority items</em>
              </div>
                ...
                {items.slice(0, 5).map((item) => {
                  ...
                  return (
                    <li key={item.id}>
                      ...
                      <code>{item.url}</code>
                      <small data-manual-signoff-status={result?.status || "not_tested"}>
                        Status: {result?.status || "not_tested"} · Priority: {item.priority}
                      </small>
                    </li>
                  );
                })}
      ...
      <div className="lo-manualSignoff__decision">
        <div>
          <strong>Browser signoff status</strong>
          <p>
            The manual browser signoff results snapshot is active. Use it to track final page-by-page visual, navigation, runtime, laptop, and mobile checks.
          </p>
        </div>
...
function RoleSecurityDashboardSurface() {
  const snapshot = roleSecurityUxResults || {};
  const model = snapshot.model || buildRoleSecurityUxModel({
    source: "lord_outcomes_home_fallback",
    results: [],
  });

  const rows = buildRoleSecurityUxRows(model);
  const generatedAt = snapshot.generatedAt
    ? new Date(snapshot.generatedAt).toLocaleString()
    : "Not generated";

  const groupedChecks = SHS_ROLE_SECURITY_UX_REGISTRY.reduce((groups, check) => {
    groups[check.group] ||= [];
  ...
  return (
    <section className={["lo-roleSecurity", roleSecurityUxStatusClass(model.roleSecurityStatus)].join(" ")}>
      <div className="lo-roleSecurity__header">
        <div>
          <p className="lo-roleSecurity__eyebrow">SHS V1 Role/Security UX Checks</p>
          <h2>{model.roleSecurityLabel}</h2>
          <span>{model.recommendedNextAction}</span>
        </div>

        <div className="lo-roleSecurity__score">
          <small>Checks Tested</small>
          <strong>{model.summary.testedChecks}/{model.summary.totalChecks}</strong>
          <em>{model.summary.notTestedChecks} not tested</em>
        </div>
      </div>

      <div className="lo-roleSecurity__snapshotMeta">
        <div>
          <strong>Role/security results snapshot</strong>
          <span>Generated: {generatedAt}</span>
        </div>
        <div>
          <strong>Mode</strong>
      ...
      <div className="lo-roleSecurity__grid">
        {rows.map(([label, value]) => (
          <article key={label}>
            <span>{label}</span>
            <strong>{value}</strong>
      ...
      <div className="lo-roleSecurity__groups">
        <div className="lo-roleSecurity__groupsHeader">
          <strong>Role/security check groups</strong>
          <span>Auth, protected routes, admin-only pages, hub access, reporting access, role navigation, and runtime safety.</span>
        </div>

        <div className="lo-roleSecurity__groupList">
          {Object.entries(groupedChecks).map(([group, checks]) => (
            <article key={group}>
              <div>
                <span>{SHS_ROLE_SECURITY_GROUPS[group] || group}</span>
                <strong>{checks.length} checks</strong>
                <em>{checks.filter((check) => check.priority === 1).length} priority checks</em>
              </div>
                ...
                {checks.slice(0, 5).map((check) => {
                  ...
                  return (
                    <li key={check.id}>
                      ...
                      <small data-role-security-status={result?.status || "not_tested"}>
                        Status: {result?.status || "not_tested"} · Access: {check.expectedAccess}
                      </small>
                    </li>
                  );
                })}
      ...
      <div className="lo-roleSecurity__decision">
        <div>
          <strong>Role/security UX status</strong>
          <p>
            The dashboard now reads the real role/security results snapshot. Manual browser checks can update this file check by check.
          </p>
        </div>
...
function RouteSmokeDashboardSurface() {
  const snapshot = routeSmokeTestResults || {};
  const model = snapshot.model || buildRouteSmokeTestModel({
    source: "lord_outcomes_home_fallback",
    results: [],
  });

  const rows = buildRouteSmokeRows(model);
  const generatedAt = snapshot.generatedAt
    ? new Date(snapshot.generatedAt).toLocaleString()
    : "Not generated";

  const groupedRoutes = SHS_ROUTE_SMOKE_REGISTRY.reduce((groups, route) => {
    groups[route.group] ||= [];
  ...
  return (
    <section className={["lo-routeSmoke", routeSmokeStatusClass(model.routeSmokeStatus)].join(" ")}>
      <div className="lo-routeSmoke__header">
        <div>
          <p className="lo-routeSmoke__eyebrow">SHS V1 Route Smoke Testing</p>
          <h2>{model.routeSmokeLabel}</h2>
          <span>{model.recommendedNextAction}</span>
        </div>

        <div className="lo-routeSmoke__score">
          <small>Routes Tested</small>
          <strong>{model.summary.testedRoutes}/{model.summary.totalRoutes}</strong>
          <em>{model.summary.notTestedRoutes} not tested</em>
        </div>
      </div>

      <div className="lo-routeSmoke__snapshotMeta">
        <div>
          <strong>Route results snapshot</strong>
          <span>Generated: {generatedAt}</span>
        </div>
        <div>
          <strong>Mode</strong>
      ...
      <div className="lo-routeSmoke__grid">
        {rows.map(([label, value]) => (
          <article key={label}>
            <span>{label}</span>
            <strong>{value}</strong>
      ...
      <div className="lo-routeSmoke__groups">
        <div className="lo-routeSmoke__groupsHeader">
          <strong>Route groups</strong>
          <span>Public pages, protected pages, hub, reporting, exchange, and auth/security routes.</span>
        </div>

        <div className="lo-routeSmoke__groupList">
          {Object.entries(groupedRoutes).map(([group, routes]) => (
            <article key={group}>
              <div>
                <span>{SHS_ROUTE_SMOKE_GROUPS[group] || group}</span>
                <strong>{routes.length} routes</strong>
                <em>{routes.filter((route) => route.priority === 1).length} priority routes</em>
              </div>
                ...
                {routes.slice(0, 5).map((route) => {
                  ...
                  return (
                    <li key={route.id}>
                      ...
                      <code>{route.url}</code>
                      <small data-route-status={result?.status || "not_tested"}>
                        Status: {result?.status || "not_tested"} · Access: {route.expectedAccess}
                      </small>
                    </li>
                  );
                })}
      ...
      <div className="lo-routeSmoke__decision">
        <div>
          <strong>Smoke test status</strong>
          <p>
            The dashboard now reads the real route smoke results snapshot. Manual browser tests can update this file route by route.
          </p>
        </div>
...
export default function LordOutcomesHome(){
  return (
    <>
        <ManualBrowserSignoffDashboardSurface />
        <RoleSecurityDashboardSurface />
        <RouteSmokeDashboardSurface />
