...
const BALLOT = [
  {
    ...
    candidates: [
      { id: "c1", name: "Alex Carter",  party: "Unity",   bio: "Former council member, urban renewal plan." },
      { id: "c2", name: "Jordan Reyes", party: "Forward", bio: "Education advocate, transit upgrades." },
      { id: "c3", name: "Taylor Kim",   party: "Civic",   bio: "Small business owner, safety-first." },
    ],
  },
  {
    id: "race-treasurer",
    ...
    instructions: "Select one (1).",
    candidates: [
      { id: "t1", name: "Morgan Singh", party: "Civic",   bio: "CPA, transparency platform." },
      { id: "t2", name: "Riley Brooks", party: "Forward", bio: "Fintech background, open data." },
    ],
  },
];
...
function addAttestation(lessonId){
  ...
  if (!arr.some(a => a.eventType==="micro-lesson-complete" && a.lessonId===lessonId)) {
    ...
    try { window.dispatchEvent(new StorageEvent("storage", { key: KEY_ATTEST, newValue: "updated" })); } catch {}
...
function RaceCard({ race, value, onChange }) {
  return (
    <section className="card card--pad" role="group" aria-labelledby={`${race.id}-label`}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <h3 id={`${race.id}-label`} style={{ margin: 0 }}>{race.title}</h3>
        <span style={{ fontSize: 12, opacity: .7 }}>{race.instructions}</span>
      </div>
      <div style={{ display: "grid", gap: 8, marginTop: 8 }}>
        {race.candidates.map(c => (
          <label key={c.id} className="sh-card-radio" style={{ display: "flex", gap: 12, alignItems: "flex-start", cursor: "pointer" }}>
            <input
              type="radio"
              name={race.id}
              value={c.id}
            ...
            <div>
              <div style={{ display: "flex", gap: 8, alignItems: "baseline" }}>
                ...
                <span className="sh-badge is-ghost">{c.party}</span>
              </div>
              {c.bio && <div style={{ fontSize: 13, opacity: .8, marginTop: 2 }}>{c.bio}</div>}
            </div>
          </label>
        ))}
      </div>
...
export default function Elections() {
  ...
  const castVote = () => {
    ...
    try {
      ...
      try { localStorage.setItem(KEY_VOTE_FLAG, "1"); } catch {}
      ...
      try { addPoints?.(10); } catch {}
      if (!(badges || []).includes("micro:elections-howto")) {
        try { addBadge?.("micro:elections-howto"); } catch {}
      }
      try {
  ...
  return (
    <section className="crb-main" aria-labelledby="elex-title">
      <header className="db-head">
        <div>
          <h1 id="elex-title" className="db-title">Elections (Practice)</h1>
        <p className="db-subtitle">Cast a practice ballot. Your selection is stored locally for learning analytics.</p>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <Link className="sh-btn is-ghost" to="/proposals">View Proposals</Link>
          <Link className="sh-btn is-ghost" to="/dashboard-ns">Northstar</Link>
          <RewardsChip />
        </div>
      </header>

      <div className="db-grid" style={{ gridTemplateColumns: "1fr", rowGap: 12 }}>
        {BALLOT.map(r => (
          <RaceCard key={r.id} race={r} value={selections[r.id]} onChange={onPick} />
        ))}

        <section className="card card--pad" aria-label="Submit">
          <div style={{ display: "flex", gap: 8 }}>
            <button className="sh-btn" disabled={!allSelected || saving} onClick={castVote}>
              {saving ? "Submitting…" : "Cast Vote"}
            </button>
            <Link className="sh-btn is-ghost" to="/dashboard">Cancel</Link>
            {!allSelected && (
              <span style={{ marginLeft: "auto", fontSize: 12, opacity: .7 }}>
                Select one candidate in each race to enable voting.
              </span>
            )}
          </div>
        </section>

        <aside className="card card--pad" aria-label="Your Last Votes">
          <strong style={{ display: "block", marginBottom: 8 }}>Recent Practice Votes</strong>
          <VoteLog />
        </aside>
      </div>
    </section>
...
function VoteLog() {
  ...
  React.useEffect(() => {
    const onStorage = (e) => {
      if (!e || e.key == null) { setRows(getVotes().slice().reverse()); return; }
  ...
  if (!rows.length) return <div style={{ opacity: .7, fontSize: 13 }}>No votes yet.</div>;

  return (
    <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
      {rows.slice(0, 6).map((r, i) => (
        <li key={i} style={{ border: "1px solid var(--ring,#eee)", borderRadius: 10, padding: "8px 10px", background: "#fff" }}>
          <div style={{ fontSize: 12, opacity: .7 }}>{new Date(r.at).toLocaleString()}</div>
          <div style={{ display: "grid", gap: 4, marginTop: 6 }}>
            {Object.entries(r.selections).map(([raceId, candId]) => (
              <div key={raceId} style={{ fontSize: 13 }}>
                <strong>{labelForRace(raceId)}:</strong> {labelForCandidate(raceId, candId)}
              </div>
            ))}
          </div>
        </li>
...
function labelForCandidate(raceId, candId) {
  ...
  return cand ? `${cand.name} (${cand.party})` : candId;
}
