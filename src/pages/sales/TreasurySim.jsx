// src/pages /sales/TreasurySim.jsx
...
import PortfolioHint from "@/components /sales/PortfolioHint.jsx"; // ⬅️ NEW
...
function uid(prefix="snap"){ return `${prefix}_` + Date.now().toString(36) + Math.random().toString(36).slice(2,6); }
...
function useTreasuryStateSync(stateObj) {
  ...
  React.useEffect(() => {
    try {
      ...
      window.dispatchEvent(new StorageEvent("storage", { key: KEY_STATE, newValue: "updated" }));
    } catch {}
  }, [stateStr]);
}
...
function useSaveSnapshotInSim() {
  ...
  return React.useCallback(({ name = "Snapshot", note = "" } = {}) => {
    const curr = readJSON(KEY_STATE, null);
    if (!curr) {
      toast("No current treasury state found. Make some changes first.", { type: "warning" });
      return;
    }
    const entry = { id: uid(), name, note, state: curr, at: Date.now() };
    const next  = [entry, ...readJSON(KEY_SNAPS, [])];
    saveJSON(KEY_SNAPS, next);

    // KPI + wallet history (capped via logWallet)
    bumpKPI(KPI_TSIMS, +1);
    logWallet({ note: `Saved Treasury snapshot: ${entry.name}`, delta: 0 }); // ⬅️ REPLACED

    try { window.dispatchEvent(new StorageEvent("storage", { key: KEY_SNAPS, newValue: "updated" })); } catch {}
...
export default function TreasurySim() {
  ...
  const total = lines.reduce((s, l) => s + (Number(l.val)||0), 0);
  const balance = cap - total;
  const remaining = Math.max(0, balance);

  // Clamp + set a line
  const setLine = (id, v) => {
    const num = Math.max(0, Math.min(cap, Number(v) || 0));
    setLines(ls => ls.map(l => l.id === id ? { ...l, val: num } : l));
  };
  ...
  const liveState = React.useMemo(() => ({
    ...
    expense: total,
    balance,
    allocations: lines.map(({ id, label, val }) => ({ id, label, value: Number(val) || 0 })),
  ...
  return (
    <section className="crb-main" aria-labelledby="ts-title">
      <header className="db-head">
        <div>
          <h1 id="ts-title" className="db-title">Treasury Simulator</h1>
          <p className="db-subtitle">Adjust allocations, watch your balance, and save snapshots.</p>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <button
            className="sh-btn"
            onClick={() => saveSnapshotInSim({ name: `Scenario (cap ${cap})`, note: `${lines.length} programs` })}
          >
            Save Snapshot
          </button>
          <a className="sh-btn is-ghost" href=" /sales.html#/snapshots" aria-label="Open snapshots">
            Open Snapshots →
          </a>
          <RewardsChip />
        </div>
      ...
      <div className="db-grid" style={{ gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <article className="card card--pad" aria-label="Totals">
          ...
          <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
            ...
            <span className="sh-badge is-ghost">Allocated: {total}</span>
            <span className="sh-badge" style={{ background: remaining ? "#ecfeff" : "#f0fdf4" }}>
              Remaining: {remaining}
            </span>
          </div>
        </article>

        <article className="card card--pad" aria-label="Programs">
          <strong>Programs</strong>
          <div style={{ display:"grid", gap:12, marginTop:8 }}>
            {lines.map(l => (
              <div key={l.id}>
                <div style={{ display:"flex", alignItems:"baseline", gap:8 }}>
                  <span style={{ width:160 }}>{l.label}</span>
                  <input
                    type="range"
                    min={0}
                    max={cap}
                  ...
                  />
                  <code style={{ width:64, textAlign:"right" }}>{l.val}</code>
                </div>
                {/* tiny bar */}
                <div style={{ height:6, background:"#eef2ff", borderRadius:999, marginTop:6, overflow:"hidden" }}>
                  <span
                    style={{
                      display:"block",
                      height:"100%",
                      width: `${Math.min(100, (Number(l.val||0) / Math.max(1, cap)) * 100)}%`,
          ...
          <div style={{ marginTop:10, display:"flex", gap:8, flexWrap:"wrap" }}>
            <button
              className="sh-btn"
              onClick={() => saveSnapshotInSim({ name: `Scenario (cap ${cap})`, note: `balance=${balance}` })}
            >
              Save scenario
            </button>
            <a className="sh-btn is-ghost" href=" /sales.html#/snapshots">Open snapshots →</a>
            {balance < 0 && (
              <span style={{ marginLeft:"auto", fontSize:12, color:"var(--danger,#b91c1c)" }}>
      ...
      <section className="card card--pad" style={{ marginTop: 12 }}>
        ...
        <ul style={{ marginTop: 8 }}>
          <li>Save a snapshot whenever you reach a scenario worth comparing.</li>
          <li>Open <a href=" /sales.html#/snapshots">Snapshots</a> to review, compare, and share.</li>
        </ul>
