...
const LESSON_ROOT = path.join(ROOT, "src/content/lessons/data-center-foundations-student");
const grade6 = MAP.courses.find((course) => course.courseId === "data-center-foundations");
const lessons = grade6.lessonIds.map((id) => ({
  id,
  ...JSON.parse(fs.readFileSync(path.join(LESSON_ROOT, `${id}.json`), "utf8")),
}));
...
test.describe("Grade 6 Data Center lesson routes", () => {
  ...
  test("sequential navigation reaches every Grade 6 lesson and stops honestly", async ({ page }) => {
    for (let index = 0; index < lessons.length; index += 1) {
      ...
      if (index < lessons.length - 1) {
        ...
        await expect(page.getByRole("heading", { name: lessons[index + 1].title, exact: true })).toBeVisible();
      } else {
  ...
  test("project lesson exposes instructions, rubric, and accessible submission alternatives", async ({ page }) => {
    ...
    await expect(page.getByText("paper model or diagram", { exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Assess" }).click();
    await expect(page.getByText("Systems explanation", { exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Reflect" }).click();
    await expect(page.getByText(/^Reflection:/)).toBeVisible();
  });

  test("mobile lesson layout has no horizontal overflow and keeps completion usable", async ({ page }) => {
  ...
  test("tablet lesson layout remains readable and semantically headed", async ({ page }) => {
    await page.setViewportSize({ width: 768, height: 1024 });
    await page.goto(lessonUrl(lessons[4].id), { waitUntil: "networkidle" });
    expect(await page.evaluate(() => document.body.scrollWidth > window.innerWidth + 1)).toBe(false);
    await expect(page.getByRole("heading", { level: 1, name: lessons[4].title, exact: true })).toBeVisible();
    expect(await page.locator("h1").count()).toBe(1);
  });

  test("essential completion control is keyboard operable", async ({ page }) => {
    await page.goto(lessonUrl(lessons[0].id), { waitUntil: "networkidle" });
