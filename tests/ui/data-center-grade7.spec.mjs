...
const LESSON_ROOT = path.join(ROOT, "src/content/lessons/data-center-systems-7-student");
const gradeSeven = MAP.courses.find((course) => course.courseId === "data-center-systems-7");
const lessons = gradeSeven.lessonIds.map((id) => ({ id, ...JSON.parse(fs.readFileSync(path.join(LESSON_ROOT, `${id}.json`), "utf8")) }));

function lessonUrl(id) {
  return `${BASE}/curriculum.html#/curriculum/lessons/${encodeURIComponent(id)}`;
}

test.describe("Grade 7 Data Center lesson routes", () => {
  ...
  test("sequential navigation reaches every lesson and ends without a Grade 8 link", async ({ page }) => {
  ...
  test("systems challenge exposes safe project instructions and rubric", async ({ page }) => {
    ...
    await expect(page.getByText("written scenario response with a systems diagram", { exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Assess" }).click();
    await expect(page.getByText("Systems reasoning", { exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Reflect" }).click();
    await expect(page.getByText(/^Reflection:/)).toBeVisible();
  });

  test("representative technical lessons remain usable on tablet and mobile", async ({ page }) => {
