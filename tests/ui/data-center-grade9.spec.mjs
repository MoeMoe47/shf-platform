...
const LESSON_ROOT = path.join(ROOT, "src/content/lessons/data-center-technical-foundations-9-student");
const course = MAP.courses.find((item) => item.courseId === "data-center-technical-foundations-9");
...
test.describe("Grade 9 Data Center lesson routes", () => {
  ...
  test("sequential navigation reaches every lesson and stops before Grade 10", async ({ page }) => {
  ...
  test("technical project exposes safe deliverables and documentation rubric", async ({ page }) => {
    ...
    await expect(page.getByText("structured technical report from a browser simulation", { exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Assess" }).click();
    await expect(page.getByText("Safe technical reasoning", { exact: true })).toBeVisible();
  ...
  test("representative technical lessons remain usable on tablet and mobile", async ({ page }) => {
    ...
    await page.goto(lessonUrl(lessons[3].id), { waitUntil: "networkidle" });
